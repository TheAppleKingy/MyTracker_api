...
class AlchemyTaskRepository(TaskRepositoryInterface):
    ...
    async def get_by_id(self, task_id: int) -> Optional[Task]:
        return await self._session.scalar(
            select(Task).where(Task.id == task_id).options(
                selectinload(Task.parent, recursion_depth=MAX_DEPTH)
            )
        )
    ...
    async def get_user_tasks_trees(self, user_id: int) -> list[Task]:
        res = await self._session.scalars(
            select(Task).where(
                Task.user_id == user_id, Task.parent_id == None
            ).options(selectinload(Task.subtasks, recursion_depth=MAX_DEPTH))
        )
    ...
    async def get_task_tree(self, from_task_id: int) -> Optional[Task]:
        return await self._session.scalar(
            select(Task).where(Task.id == from_task_id).options(
                selectinload(Task.subtasks, recursion_depth=MAX_DEPTH)
            )
        )
